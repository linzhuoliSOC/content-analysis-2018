...
#SBATCH --time=$TIME
#SBATCH -J nb_server
#SBATCH -o nb_sessions/nb_session_%J.out

# LOAD A PYTHON MOUDLE WITH JUPYTER
module load Anaconda3/5.0.0.1
pip install -U --user /project2/macs60000/lucem_illud
#
...
echo " "
echo -n "    WAITING FOR SERVER TO START (CTRL-C then scancel $JOBID TO HALT)..."
    NB_ADDRESS=$(cat nb_sessions/nb_session_${JOBID}.out | grep "] http" | awk -F 'http' '{print $2}' )
  ...
  echo "    $TIMELEFT"
  echo "    SESSION LOG WILL BE STORED IN nb_sessions/nb_session_${JOBID}.out  "
  echo "  --------------------------------------------------------------------"
  echo "  "
